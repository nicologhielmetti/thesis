...
#             y_act['y'].append(i_model.predict(X_test).astype(np.float32))
#             y_act['name'].append(l.name)
#     return y_act
# 
# 
# act = get_activations(model, X_test[:100])
# actq = get_activations(qmodel, X_test[:100])
...
test_perf = qmodel.evaluate(x=X_test, y=y_test, verbose=1, workers=12, use_multiprocessing=True,
